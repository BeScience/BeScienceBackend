...
public class ExceptionHandleFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            RootException exception = new RootException(ErrorCode.INVALID_TOKEN, "토큰이 변조됬습니다."+e.getMessage(), "토큰이 변조됬습니다.");
            returnErrorResponse(request, response, exception);
        } catch (ExpiredJwtException e) {
            ExpiredTokenException exception = new ExpiredTokenException("토큰이 만료됬습니다.", "Access Token 만료됨.");
            returnErrorResponse(request, response, exception);
        } catch (Exception e) {
            log.error("[FILTER] 예상치 못한 오류 발생: {}", e.getMessage());
            RootException rootException = new RootException(ErrorCode.UNKNOWN_ERROR, "예상치 못한 오류 발생: "+ e.getMessage(), "예상치 못한 쌈@뽕한 오류 발생! 백엔드에게 이 메시지를 전해주세여: "+ e.getMessage());
